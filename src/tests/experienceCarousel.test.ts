...
describe("initExperienceCarousel", () => {
  ...
  it("next: hace scrollBy si no está al final; hace scrollTo(0) si está al final", () => {
    ...
    Object.defineProperty(carousel, "scrollLeft", { value: 200, writable: true, configurable: true });
  ...
  it("autoplay: cuando el progreso llega a 100% avanza el carrusel y resetea el progreso", () => {
    ...
    expect(
      (carousel.scrollBy as any).mock.calls.length +
        (carousel.scrollTo as any).mock.calls.length
    ).toBeGreaterThan(0);
